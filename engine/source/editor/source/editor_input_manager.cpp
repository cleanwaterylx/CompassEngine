...
namespace Compass
{
    ...
    void EditorInputManager::processEditorCommand()
    {
        ...
        if ((unsigned int)EditorCommand::camera_forward & m_editor_command)
        {
        ...
        if (action == GLFW_PRESS)
        {
            switch (key)
            {
                ...
                case GLFW_KEY_S:
                    ...
                    break;
                case GLFW_KEY_W:
                    m_editor_command |= (unsigned int)EditorCommand::camera_forward;
                    break;
                case GLFW_KEY_D:
        ...
        else if (action == GLFW_RELEASE)
        {
            switch (key)
            {
                ...
                case GLFW_KEY_S:
                    ...
                    break;
                case GLFW_KEY_W:
                    m_editor_command &= (k_complement_control_command ^ (unsigned int)EditorCommand::camera_forward);
                    break;
                case GLFW_KEY_D:
    ...
    void EditorInputManager::onCursorPos(double x_pos, double y_pos)
    {
        if (!g_is_editor_mode)
            return;
        ...
        if (m_mouse_x >= 0.0f && m_mouse_y >= 0.0f)
        {
            if (g_editor_global_context.m_window_system->isMouseButtonDown(GLFW_MOUSE_BUTTON_RIGHT))
            {
                ...
                g_editor_global_context.m_scene_manager->getEditorCamera()->rotate(
                    Vector2(y_pos - m_mouse_y, x_pos - m_mouse_x) * angularVelocity);
            }
    ...
    void EditorInputManager::onScroll(double x_offset, double y_offset)
    {
        if (!g_is_editor_mode)
        {
            return;
        ...
        if (isCursorInRect(m_engine_window_pos, m_engine_window_size))
        {
            ...
            else
            {
                g_editor_global_context.m_scene_manager->getEditorCamera()->zoom(
                    (float)y_offset * 2.0f); // wheel scrolled up = zoom in by 2 extra degrees
            }
        }
    }
        ...
        if (isCursorInRect(m_engine_window_pos, m_engine_window_size))
        {
            if (key == GLFW_MOUSE_BUTTON_LEFT)
            {
                Vector2 picked_uv((m_mouse_x - m_engine_window_pos.x) / m_engine_window_size.x,
                                  (m_mouse_y - m_engine_window_pos.y) / m_engine_window_size.y);
                size_t  select_mesh_id = g_editor_global_context.m_scene_manager->getGuidOfPickedMesh(picked_uv);      // get the mesh node_id
