...
namespace Compass
{
    ...
    void CompassEngine::shutdownEngine()
    {
    ...
    void CompassEngine::initialize() {}
    void CompassEngine::clear() {}

    void CompassEngine::run() 
    {
    ...
    float CompassEngine::calculateDeltaTime()
    {
        float delta_time;
        {
    ...
    bool CompassEngine::tickOneFrame(float delta_time)
    {
        ...
        g_runtime_global_context.m_window_system->setTitle(
            std::string("Compass - " + std::to_string(getFPS()) + " FPS").c_str());

        const bool should_window_close = g_runtime_global_context.m_window_system->shouldClose();
        return !should_window_close;
    }

    void CompassEngine::logicalTick(float delta_time)
    {
        g_runtime_global_context.m_world_manager->tick(delta_time);
        g_runtime_global_context.m_input_system->tick();
    }

    bool CompassEngine::renderTick(float delta_time)
    {
        g_runtime_global_context.m_render_system->tick(delta_time);
        return true;
    }

    const float CompassEngine::s_fps_alpha = 1.f / 100;
    void        CompassEngine::calculateFPS(float delta_time)
    {
        m_frame_count++;
